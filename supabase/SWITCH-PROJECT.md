...
**แก้ปัญหา: "Entrypoint path does not exist ... /source/index.ts"**

ระบบ deploy คาดว่าไฟล์หลักในหน้า editor ต้องชื่อ `index.ts` เป๊ะ ๆ — ถ้าเจอ error นี้:
1. ดูแถบไฟล์/แท็บเหนือช่องโค้ด ต้องมีไฟล์ชื่อ **`index.ts`** — ถ้าไม่มี กดปุ่มสร้างไฟล์ใหม่ (ไอคอน +) ตั้งชื่อ `index.ts` แล้ววางโค้ดในไฟล์นั้น
2. ถ้าไฟล์ชื่ออื่น (เช่น `untitled`) ให้ rename เป็น `index.ts`
3. ถ้ายังไม่หาย: ลบฟังก์ชันตัวที่เสียทิ้ง (เข้าหน้าฟังก์ชัน → Delete) แล้วสร้างใหม่ตั้งแต่ต้น
   โดย**พิมพ์ชื่อฟังก์ชันเอง** (อย่าวางจาก clipboard — อาจมีช่องว่างติดมา) → วางโค้ด → Deploy
